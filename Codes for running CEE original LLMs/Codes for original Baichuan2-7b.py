...
model = AutoModelForCausalLM.from_pretrained("/root/autodl-tmp/Baichuan2-7B-Chat", device_map="auto", torch_dtype=torch.bfloat16, trust_remote_code=True)
...
for year in years:
  ...
  for i in range(len(Questions)):
    ...
    {"Question":Questions[i],"Correct_Answer":Answers[i],"Answer1":answer1,"Score1":final_score(Questions[i],answer1,Answers[i])}])
      ...
      print("No Question",i+1,"\n","Right_Answer:",Answers[i],"\nAnswer_from_original_baichuan2-7b:",answer1.strip().replace("\n",""))

      df = pd.concat([df, df2], axis=0)

  save_df_to_excel(df, "Answers_from_Original_baichuan2_7B_EE_"+year+".xlsx", "sheet1")
